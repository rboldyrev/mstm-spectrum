#! /usr/bin/env python
#
# GUI модуль был впервые создан с помощью PAGE версии 4.9
# В сочетании с Tcl версии 8.6
...
import time  # для тестирования заставки

def vp_start_gui():
    '''Точка входа, когда модуль является основной программой.'''
    global val, w, root
    root = Tk()
...
def create_MSTM_studio(root, *args, **kwargs):
    '''Точка входа, когда модуль импортируется другой программой.'''
    global w, w_win, rt
    rt = root
    w = Toplevel (root)
    top = MSTM_studio (w)
...
class MSTM_studio:
    ...
    splash_time = 0.5  # время показа заставки, секунды

    def __init__(self, top=None):
        '''Этот класс настраивает и заполняет главное окно.
           top - это главное окно.'''
        top.withdraw()
        ...
        if sys.platform in ('win32', 'win64'):
            ...
            self.style.theme_use('default')  # это исправляет отображение в MacOS
        ...
        self.canvas.bind('<Button-4>',   sup.mouse_wheel)  # для Linux
        self.canvas.bind('<Button-5>',   sup.mouse_wheel)  # для Linux
        self.canvas.bind('<MouseWheel>', sup.mouse_wheel)  # для Windows
        ...
        self.cbContribs = []
        self.edContribs = []  # на самом деле это будет список списков [[]]
        self.btPlotsContrib = []
        ...
        time_delta = time.time() - time_start  # в секундах
    ...
    def __adjust_sash0(self, event):  # загадочные функции, оставшиеся от предыдущих цивилизаций
        paned = event.widget
        pos = [220, 575, ]
        i = 0
        for sash in pos:
...
# Следующий код добавлен для обеспечения работы Scrolled виджетов, которые вы указали.
class AutoScroll(object):
    '''Настройка полос прокрутки для виджета.'''

    def __init__(self, master):
        #  Rozen. Добавлены блоки try-except, чтобы этот класс
        #  можно было использовать для виджета с прокруткой, где вертикальная
        #  прокрутка не поддерживается. 5/7/14.
        try:
        ...
        # Копирование методов геометрии из master (взято из ScrolledText.py)
        if py3:
    ...
    def _autoscroll(sbar):
        '''Скрывать и показывать полосу прокрутки по мере необходимости.'''
...
def _create_container(func):
    '''Создает ttk Frame с заданным master и использует этот новый frame для
    размещения полос прокрутки и виджета.'''
...
class ScrolledTreeView(AutoScroll, ttk.Treeview):
    '''Стандартный виджет ttk Treeview с полосами прокрутки, которые будут
    автоматически показываться/скрываться по мере необходимости.'''
    @_create_container
