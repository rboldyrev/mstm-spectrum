...
class NearField(SPR):
    '''
    Расчет карты распределения поля на фиксированной длине волны
    '''
    ...
    def __init__(self, wavelength):
        ...
        self.set_plane()
        self.paramDict['calculate_near_field'] = 1  # выполнить расчет

    def set_plane(self, plane='zx', hmin=-10., hmax=10.,
                  vmin=-10., vmax=10., step=1., offset=0.):
        '''
        Определение плоскости и сетки для расчета ближнего поля.

        plane: одна из 'yz'|'zx'|'xy'
        hmin, hmax, vmin, vmax: горизонтальные и вертикальные размеры
        step:   размер шага сетки
        ...
        if plane == 'yx':
            plane = 'xy'
        # 1: плоскость y - z;  2: плоскость z - x;  3: плоскость x - y
        if plane == 'yz':
            ...
            raise Exception('Некорректное указание плоскости! \n %s' % plane)
        # TODO: расширить для расчета комплексного вектора E = 1,
        #       комплексных векторов E и H = 2
        ...
        print('Сетка для расчета поля: %ix%i' % (self.nh, self.nv))

    def simulate(self):
        '''
        Запуск кода MSTM для создания 2D карты распределения поля.
        '''
        ...
        with tempfile.TemporaryDirectory() as tmpdir:  # СКОПИРОВАНО ИЗ РОДИТЕЛЬСКОГО КЛАССА!
            print('Используется временная директория: %s' % tmpdir)
            outFID = open(os.path.join(tmpdir, 'scriptParams.inp'), 'w')
            outFID.write('begin_comment\n')
            outFID.write('**********************************\n')
            outFID.write('  Входные данные MSTM для расчета ближнего поля\n')
            outFID.write('  Сгенерировано python-скриптом\n')
            outFID.write('  %s\n' %
            ...
            with open(fn) as fout:
                fout.readline()  # пропустить первую строку
                nsph = int(fout.readline().strip())  # количество сфер в плоскости
    ...
    def write(self, filename):
        ''' сохранение данных поля в текстовый файл '''
    ...
    def plot(self, fig=None, axs=None, caxs=None):
        '''
        Отображение 2D распределения поля
        ...
        axs:
            оси matplotlib
        caxs:
            оси matplotlib для цветовой шкалы
